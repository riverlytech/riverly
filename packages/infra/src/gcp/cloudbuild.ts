...
export class CloudBuildGitHubDeployer implements IGitHubDeployer {
  ...
  constructor(gcpConfig: GCPConfig, cloudBuildConfig: GCPBuildConfig, dryRun = true, logBuildSpec = true) {
    ...
    this.dryRun = dryRun;
    this.logBuildSpec = logBuildSpec;
  }

  async deploy(params: GitHubSourceDeployer): Promise<DeploymentEvent> {
    console.info(
      {
    ...
    const buildDefinition: protos.google.devtools.cloudbuild.v1.IBuild = {
      ...
      substitutions: {
        ...
        _REGION: this.gcpConfig.GCP_REGION,
      },
      options: {
      ...
      steps: [
        {
          ...
          id: "FETCH_SECRET",
          entrypoint: "bash",
          args: ["-c", fetchSecretScript(this.gcpConfig)],
        },
        {
