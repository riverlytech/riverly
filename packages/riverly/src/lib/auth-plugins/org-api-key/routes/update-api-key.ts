  ...
  return createAuthEndpoint(
    "/org-api-key/update",
    {
      ...
      body: z.object({
        ...
        userId: z.coerce
          ...
          .optional(),
        organizationId: z.coerce.string().meta({
          description:
            'The id of the organization which the api key belongs to. server-only. Eg: "some-org-id"',
        }),
        name: z
          .string()
          .meta({
    ...
    async (ctx) => {
      ...
      if (metadata !== undefined) {
        ...
        // we need this to be a string to save into DB.
        newValues.metadata =
          // @ts-expect-error
          schema.apikey.fields.metadata.transform.input(metadata) as Record<string, any> | null;
      }
