...
import type { ApiKeyOptions } from "./ty";
import { apiKeySchema } from "./schema";
import type { BetterAuthPlugin } from "better-auth";
import { APIError, createAuthMiddleware } from "better-auth/api";
import { getIp, getDate } from "./utils";
...
export const ERROR_CODES = {
  INVALID_MEMBER: "User is not a member of the organization",
...
export const orgApiKey = (options?: ApiKeyOptions) => {
  const opts = {
    ...options,
    ...
    rateLimit: {
      enabled: options?.rateLimit?.enabled === undefined ? true : options?.rateLimit?.enabled,
      ...
      before: [
        {
          ...
          handler: createAuthMiddleware(async (ctx) => {
            ...
            deleteAllExpiredApiKeys(ctx.context).catch((err: any) => {
